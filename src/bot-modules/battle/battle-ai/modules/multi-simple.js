/**
 * Ingame (No Status) for Multiples
 *
...
exports.setup = function (Data) {
	...
	function getSwitchAverage(battle, s) {
		...
		for (let i = 0; i < p.moves.length; i++) {
			...
			for (let j = 0; j < targets.length; j++) {
				...
				pokeB.supressedAbility = targets[j].pokemon.supressedAbility;
				if (targets[j].pokemon.item === "&unknown") {
					pokeB.item = null;
				} else {
					pokeB.item = targets[j].pokemon.item;
				}
				if (!targets[j].pokemon.supressedAbility && !battle.conditions["neutralizinggas"]) {
	...
	function moveIsRedirectedImmune(battle, move, pokeA, conditionsA) {
		...
		for (let i = 0; i < targets.length; i++) {
			...
			pokeB.supressedAbility = targets[i].pokemon.supressedAbility;
