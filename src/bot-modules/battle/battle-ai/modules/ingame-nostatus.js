...
exports.setup = function (Data) {
	...
	function getSwitchAverage(battle, s) {
		...
		for (let i = 0; i < p.moves.length; i++) {
			...
			for (let j = 0; j < targets.length; j++) {
				...
				pokeB.tera = targets[j].tera;
				pokeB.timesHit = targets[j].timesHit;
				pokeB.supressedAbility = targets[j].supressedAbility;
	...
	function moveIsRedirectedImmune(battle, move, pokeA, conditionsA) {
		...
		for (let i = 0; i < targets.length; i++) {
			...
			pokeB.tera = targets[i].tera;
			pokeB.timesHit = targets[i].timesHit;
			pokeB.supressedAbility = targets[i].supressedAbility;
	...
	function evaluateMoveDecision(battle, desEnv, des, act) {
		...
		for (let i = 0; i < targets.length; i++) {
			...
			if (pokeB.ability && pokeB.ability.id === "commander" && conditionsB.volatiles["activeability"] && battle.foe.active.filter(p => p.species === "Dondozo" && !p.fainted).length > 0) {
