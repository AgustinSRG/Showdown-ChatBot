...
App.server.setHandler('quotejoke', (context, parts) => {
	...
	html += '<div align="center"><h2>Quotes and Jokes</h2>';
	html += '<a class="submenu-option' + (opt in {'': 1, 'quotes': 1} ? '-selected' : '') + '" href="/quotejoke/quotes/">Quotes</a>';
	html += ' | ';
	html += '<a class="submenu-option' + (opt in {'jokes': 1} ? '-selected' : '') + '" href="/quotejoke/jokes/">Jokes</a>';
	html += '</div>';
	html += '<hr />';
	...
	case '':
	case 'quotes':
		quotesHandler(context, html);
		break;
	case 'jokes':
		jokesHandler(context, html);
		break;
	default:
		context.endWith404();
	}
