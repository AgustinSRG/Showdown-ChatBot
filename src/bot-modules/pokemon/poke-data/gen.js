...
function getGeneration(name, App) {
	...
	if (id === 'metronome') {
		...
		else return {gen: 6, name: pokedex[id].species};
	} else if (movedex[id]) {
		if (movedex[id].num <= 165) return {gen: 1, name: movedex[id].name};
		...
		else if (movedex[id].num <= 617) return {gen: 6, name: movedex[id].name};
		else return {gen: 'CAP', name: movedex[id].name};
		...
		else return {gen: 6, name: abilities[id].name};
	} else if (items[id]) {
