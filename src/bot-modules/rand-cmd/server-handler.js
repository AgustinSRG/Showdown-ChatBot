/**
 * Server Handler: Random Commands
 */
...
exports.setup = function (App) {
	/* Permissions */
	App.server.setPermission('randcmds', 'Permission for managing random commands');

	/* Menu Options */
	App.server.setMenuOption('randcmds', 'Random&nbsp;Commands', '/randcmds/', 'randcmds', -2);

	/* Handlers */
	App.server.setHandler('randcmds', (context, parts) => {
		if (!context.user || !context.user.can('randcmds')) {
			context.endWith403();
			return;
		}

		let submenu = new SubMenu("Random&nbsp;Commands", parts, context, [
			{id: 'commands', title: 'Commands', url: '/randcmds/', handler: commandsHandler},
			{id: 'aliases', title: 'Aliases', url: '/randcmds/aliases/', handler: aliasesHandler},
		], 'commands');
	...
	function commandsHandler(context, html) {
		...
		context.endWithWebPage(html, {title: "Random Commands - Showdown ChatBot"});
	}
	...
	function aliasesHandler(context, html) {
		...
		context.endWithWebPage(html, {title: "Random Commands (Aliases) - Showdown ChatBot"});
	}
};
