...
class ChessGame {
	constructor(room, players, App) {
		this.room = room;
		this.players = players; // [white, black]
		this.App = App;
		this.board = this.initializeBoard();
		this.currentPlayer = 0; // 0 = white, 1 = black
		this.moveHistory = [];
		this.gameOver = false;
		this.winner = null;
	...
	getPieceColor(piece) {
		if (this.isWhitePiece(piece)) return 0; // white
		if (this.isBlackPiece(piece)) return 1; // black
		return -1;
	}
	...
	isValidPieceMove(piece, from, to) {
		...
		switch (pieceType) {
			...
			case 'q':
				return this.isValidRookMove(from, to, deltaRow, deltaCol) || this.isValidBishopMove(from, to, deltaRow, deltaCol);
			case 'k':
	...
	isInCheck(playerColor, board) {
		...
		for (let r = 0; r < 8; r++) {
			for (let c = 0; c < 8; c++) {
				const pc = useBoard[r][c];
				if (pc && this.getPieceColor(pc) !== playerColor) {
					if (this.canPieceAttack(pc, { row: r, col: c }, kingPos, useBoard)) {
						return true;
					}
				}
			}
	...
	canPieceAttack(piece, from, to, board) {
		...
		switch (pieceType) {
			...
			case 'r':
				return (deltaRow === 0 || deltaCol === 0) && this.isPathClear(from, to, useBoard);
			case 'n':
				return (Math.abs(deltaRow) === 2 && Math.abs(deltaCol) === 1) || (Math.abs(deltaRow) === 1 && Math.abs(deltaCol) === 2);
			case 'b':
				return Math.abs(deltaRow) === Math.abs(deltaCol) && this.isPathClear(from, to, useBoard);
			case 'q':
				return ((deltaRow === 0 || deltaCol === 0) || Math.abs(deltaRow) === Math.abs(deltaCol)) && this.isPathClear(from, to, useBoard);
			case 'k':
	...
	makeMove(move, board) {
		...
		const piece = useBoard[from.row][from.col];
		if (PIECE_VALUES[piece].toLowerCase() === 'p' && this.enPassant && to.row === this.enPassant.row && to.col === this.enPassant.col) {
			const capturedPawnRow = this.currentPlayer === 0 ? to.row + 1 : to.row - 1;
			useBoard[capturedPawnRow][to.col] = null;
		}
		if (PIECE_VALUES[piece].toLowerCase() === 'k' && Math.abs(to.col - from.col) === 2) {
			const isKingSide = to.col > from.col;
			const rookFromCol = isKingSide ? 7 : 0;
			const rookToCol = isKingSide ? 5 : 3;
			const rook = useBoard[from.row][rookFromCol];
			useBoard[from.row][rookToCol] = rook;
			useBoard[from.row][rookFromCol] = null;
		}
		useBoard[to.row][to.col] = piece;
		useBoard[from.row][from.col] = null;
		if (promotion && PIECE_VALUES[piece].toLowerCase() === 'p') {
			...
			useBoard[to.row][to.col] = promotedPieces[promotion];
		}
	}
	...
	updateGameState(move) {
		...
		if (pieceType === 'r') {
			const color2 = this.currentPlayer === 0 ? 'white' : 'black';
			if (from.col === 0) this.castling[color2].queenside = false;
			if (from.col === 7) this.castling[color2].kingside = false;
		}
				...
				if (piece && this.getPieceColor(piece) === this.currentPlayer) {
					for (let toRow = 0; toRow < 8; toRow++) {
						for (let toCol = 0; toCol < 8; toCol++) {
							const mv = { from: { row: fromRow, col: fromCol }, to: { row: toRow, col: toCol } };
	...
	getBoardHTML() {
		...
		for (let row = 0; row < 8; row++) {
			...
			for (let col = 0; col < 8; col++) {
				...
				let highlight = '';
				if (this.lastMove) {
					if ((row === this.lastMove.from.row && col === this.lastMove.from.col) || (row === this.lastMove.to.row && col === this.lastMove.to.col)) {
		...
		html += '<div style="margin-left: 15px;">';
		for (let j = 0; j < 8; j++) {
			html += '<span style="display: inline-block; width: 25px; text-align: center; font-weight: bold;">' + String.fromCharCode(97 + j) + '</span>';
		}
		html += '</div>';
		html += '</div>';
		return html;
