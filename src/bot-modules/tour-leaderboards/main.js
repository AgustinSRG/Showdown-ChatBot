...
const db = exports.db = new DataBase(Path.resolve(App.confDir, 'leaderloards.json'));

exports.data = db.data;

const tourData = exports.tourData = {};
const isOfficial = exports.isOfficial = {};
...
const getTop = exports.getTop = function (room) {
	let ladder = exports.data;
...
exports.getUserPoints = function (room, user) {
...
const generateTable = exports.generateTable = function (room) {
...
const addUser = exports.addUser = function (room, user, type, auxData) {
	let ladder = exports.data;
	let userid = Text.toId(user);
...
const parseTourResults = exports.parseTourResults = function (room, data) {
	let config = App.config.modules.tourleaderboards[room];
	if (!config) return;
	if (config.onlyOfficial && !isOfficial[room]) return;
	let results = getResults(data);
	if (!results) return;
	if (!exports.data[room]) exports.data[room] = {};
	for (let i = 0; i < results.players.length; i++) {
		addUser(room, results.players[i], 'A');
	}
	if (results.winner) {
		addUser(room, results.winner, 'W');
	}
	if (results.finalist) {
		addUser(room, results.finalist, 'F');
	}
	for (let i = 0; i < results.semiFinalists.length; i++) {
		addUser(room, results.semiFinalists[i], 'S');
	}
...
App.bot.on('line', (room, line, spl, isIntro) => {
	...
	case 'end':
		...
		parseTourResults(room, tourData[room]);
