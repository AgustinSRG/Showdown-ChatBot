...
const BattleStatIDs = exports.BattleStatIDs = {
	HP: 'hp',
	hp: 'hp',
	Atk: 'atk',
	atk: 'atk',
...
const BattleStatNames = exports.BattleStatNames = {
	hp: 'HP',
	atk: 'Atk',
	def: 'Def',
	spa: 'SpA',
...
const fastUnpackTeam = exports.fastUnpackTeam = function (buf) {
	...
	while (j >= 0) {
		...
		let template = exports.getTemplate(set.species);
...
exports.exportTeam = function (team) {
	...
	if (typeof team === 'string') {
		if (team.indexOf('\n') >= 0) return team;
		team = exports.fastUnpackTeam(team);
	}
	let text = '';
	for (let i = 0; i < team.length; i++) {
		let curSet = team[i];
		if (curSet.name !== curSet.species) {
			text += '' + curSet.name + ' (' + (exports.getTemplate(curSet.species).name || curSet.species) + ')';
		} else {
			text += '' + (exports.getTemplate(curSet.species).name || curSet.species);
		}
		if (curSet.gender === 'M') text += ' (M)';
		if (curSet.gender === 'F') text += ' (F)';
		if (curSet.item) {
			curSet.item = exports.getItem(curSet.item).name || curSet.item;
			text += ' @ ' + curSet.item;
		}
		text += "\n";
		if (curSet.ability) {
		...
		if (curSet.moves) {
			for (let j = 0; j < curSet.moves.length; j++) {
				...
				text += '- ' + (exports.getMove(move).name || move) + "\n";
			}
		}
		text += "\n";
	}
