...
class CommandsGuide {
	...
	generateHelpPage(sectionIndex, pageIndex, room) {
		...
		html += '<button class="button" name="send" value="' + Text.escapeHTML(this.getBotMessageCommand("help !,close", room)) + '">Cerrar guía</button>';
		...
		commands: {
			help: function (App) {
				...
				if (this.args[0] === "!" || !this.arg) {
					if (!roomWithBotRank) {
						return this.pmReply("Guía de comandos: https://github.com/AgustinSRG/Showdown-ChatBot/wiki/Commands-List-(Spanish)---Lista-de-Comandos");
					}
					...
					if (commandEntries.length === 0) {
						return this.pmReply("No se pudo encontrar ninguna documentación acerca del comando " + Chat.code(commandId || this.arg) + ". Intente usar " + Chat.code(this.token + this.cmd) + " sin argumentos para mostrar la lista completa de comandos.");
					}
